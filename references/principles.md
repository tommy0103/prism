...
- **Document-level budget.** The total number of visible heavy components (decision, flow, branch, compare, contrast, analogy, myth, steps, stacked-bar) on the top level should not exceed 1.5× the number of `<h2>` sections. A document with 5 sections can have up to 7–8 visible heavy components. `<p-source>` and `<p-ref>` are exempt — use as many as the code demands. Components inside `<p-collapse>` blocks are also exempt — the budget counts what the reader sees before expanding anything. When you finish drafting, count your visible heavy components and your sections. If heavy > 1.5× sections, demote the weakest ones to prose or move them into collapse blocks.
